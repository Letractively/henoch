...
{
    /// <summary>
    /// Any class who is an subject but NOT an usercontrol in the observer pattern must inherit SubjectBase for Logging.
    /// </summary>
    public abstract class SubjectBase:ISubject
    {
        ...
        public TraceContext Trace { get; protected set; }
        ...
        /// <param name="message"></param>
        public void NotifyObserver(HttpSessionState currentSession, string message)
        {
            if (NotifyLogger != null) NotifyLogger(message); //+ "\n"
            var args = new NotificationEventArgs(currentSession, message);
        ...
        public delegate string[] NotificationQueueAsync(out int threadId, int maxWorkerLoop);
