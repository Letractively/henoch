...
$(document).ready(function () {
    ...
    $(".placeOrder").click(function (event) {
        ...
        try {
            $.ajax({
                //for IIS:
                //url: "http://secure.mollie.nl/xml/ideal?a=fetch&partnerid=520197&description=Hier+een+beschrijving&reporturl=http%3A%2F%2F145.99.231.133/Adventureworks%2FDefault.aspx&returnurl=http%3A%2F%2F145.99.231.133/Adventureworks%2FThankYou.aspx&amount=1230&bank_id=9999&testmode=true&profile_key=c68eb675",
                url: "http://secure.mollie.nl/xml/ideal?a=fetch&partnerid=520197&description=Hier+een+beschrijving&reporturl=http%3A%2F%2F145.99.231.133/Adventureworks%2FDefault.aspx&returnurl=http%3A%2F%2Flocalhost:7706%2FThankYou.aspx&amount=1230&bank_id=9999&testmode=true&profile_key=c68eb675",
                success: function (data) {                    
                    xmlDoc = createXMLDocument(data);
                    var tx = $(xmlDoc).find('transaction_id')[0].nodeTypedValue;
                    $("#msg").append("<li class= 'log'>Transactie-id = " + tx + "</li>");
                    //Go tell teh server the transaction-id has been received.
                    MyServerCall(tx);
                    var url = $(xmlDoc).find('URL')[0].nodeTypedValue;
                    alert('You will be redirected to a secure page for payment.');
    ...
    var iDealCompleted = $(document).find('#statusreport');
    if (iDealCompleted.length > 0) {
        MyServerCall("completed");
    }
...
function IdealProcessing(result, context) {
    //Teh server has processed the transaction-id
}
function IDealCompleted(result, context) {
    //Teh server has processed the transaction-id
    var xmlDoc = null;
    if (result != null && result != "") {
        try {
            $.ajax({
                url: "http://secure.mollie.nl/xml/ideal?a=check&partnerid=520197&transaction_id="
                    + result + "&testmode=true",
                success: function (data) {
                    xmlDoc = createXMLDocument(data);
                    var tx = $(xmlDoc).find('payed')[0].nodeTypedValue;
                    $("#msg").append("<li class= 'log'>Transactie status payed = " + tx + "</li>");

                    if (tx == 'true')
                        $('h2').replaceWith('<h2>Thank you. You have paid.</h2>');
                    else
                        $('h2').replaceWith('<h2>Noob. You have not paid.</h2>');
                },
                error: function () {
                    alert("Error calling the web service.");
                },
        ...
        $('h2').replaceWith('<h2>Noob. The transaction has already been validated.</h2>');
}
