...
namespace DesignPatterns
{
 ...
 // The Component
    public class Component<T> : IComponent<T>
    {
        ...
        {
            IComponent<T> component = null;
            try 
	        {	        
		        component = s as IComponent<T>;
                Console.WriteLine(String.Format("Cannot remove {0} directly.", component.Uid));
	        }
	        catch (Exception ex)
	        {
                Console.WriteLine(String.Format("Cannot remove {0} directly: {1}.", s.ToString(), ex.Message));
	        }
            throw new CheckedException(ErrorType.ProcessFailure, "Composite pattern does not allow this operation.");
        }
        /// <summary>
